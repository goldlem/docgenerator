...
abstract class WordFinder implements OnWordFoundCallback {
    ...
    public String collectText(XWPFDocument doc) {
     ...
     * @param words to be searched
     */
    void findWordsInTable(XWPFDocument doc, Set<String> words) {
        this.bookmarks = words;
        for (XWPFTable t : doc.getTables()) {
            checkTable(t);
        }
        bookmarks = Set.of();
     ...
     * @param words to be searched
     */
    void findWordsInText(XWPFDocument doc, Set<String> words) {
        this.bookmarks = words;
        for (XWPFParagraph p : doc.getParagraphs()) {
            if (paragraphNotNullAndHasRuns(p)) {
                checkInParagraph(p);
            ...
            if (isRunNotNullAndNotEmpty(run)) {
                ...
                for (String bookmark: bookmarks){
                    if (text.contains(bookmark)) {
                        onWordFoundInRun(run, bookmark);
                        lastUsedRun = runIndex;
                    } else if (nextRunHasText(runs, runIndex)
                            && !nextRunsText(runs, runIndex).contains(bookmark)
                            && isWordInPreviousCurrentNextRuns(runs, lastUsedRun, runIndex, bookmark)) {
                        onWordFoundInPreviousCurrentNextRun(runs, runIndex, bookmark);
                    }
                }
            }
        }
    ...
    private boolean isWordInPreviousCurrentNextRuns(List<XWPFRun> runs, int lastUsedRun, int runIndex, String bookmark) {
